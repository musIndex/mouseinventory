...
<jsp:useBean id="newMouse" class="edu.ucsf.mousedatabase.objects.MouseRecord" scope="request"/>
...
<%
    ...
    String transgenicType = request.getParameter("transgenicType");
   	...
   	if(updateCommand == null || updateCommand.isEmpty()	|| !(updateCommand.equals("Create Record") || updateCommand.equals("Save as Incomplete")))
   	{
   		%>
	...
	PopulateMouseResult result = RecordManager.PopulateMouseDataFromRequest(newMouse,request);
	...
	String notes = "Admin submission.<br>" + newMouse.getMouseName() + " " + newMouse.getMouseType();
	...
	Properties props = MouseRecord.GetPropertiesString(submitterData,newMouse);
	int subID = DBConnect.insertAdminSubmission(request.getRemoteUser(),notes,submissionStatus,props);
  	...
  	if (updateCommand.equals("Save as Incomplete"))
	{
		...
		recordLabel = "To edit, use auto-generated submission " + 
			"<a href = 'CreateNewRecord.jsp?id=" + subID + "'>#" + subID + "</a> (in holding)";
	}
